...
#==  Touch These  ==#
#===================#
$doc_dir            = "\\kspfs03\kspgen$\KSP Technical Standards"

$mail_sender = "Documentation Updates <mgordon@ksp.ca>"
$mail_recipients = "mgordon@ksp.ca"
$date = [string](Get-Date)
$mail_subject = "Documentation Updates for $date"
...
if(-not $changed) {
    $mail_body = "No changes to documentation today"
} else {
    foreach($s in $status) {
        $mail_body += "$s`n"
    }
