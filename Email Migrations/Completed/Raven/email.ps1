...
$company_name       = "Raven Roofing"
$company_ou         = "OU=$company_name,OU=Customers,DC=internal,DC=ksphosting,DC=com"

$tenant_name        = "ravenroofingsask.onmicrosoft.com"
$company_domain     = "ravensask.com"
$tenant_account     = "admin@${tenant_name}"
...
if($CREATE_MAPPING_CSV)
{
    ...
    if($addresses.length -ne $sams.length)
    {
        ...
        # NOTE(evan): Honestly, skip the check and just manually double-check the resulting CSV.
    }
    else
    {
...
if($EXPORT_ALIASES_AND_ATTRIBUTES)
{
    ...
    ######Run this section BEFORE deleting exchange objects verify that exported csvs contain valid data###################
    ######################## Section 1#############
